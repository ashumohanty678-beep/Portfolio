...
# Design System Specification — Terminal Precision Theme
...
- **Base Canvas (`#141317` / `#0f0e12`)**: Dark, sophisticated neutral background providing a unique contextual depth.
- **Surface Containers (`#1c1b1f`, `#201f23`, `#2b292e`)**: Layered container surfaces for cards and panels.
- **Primary Steel Gray (`#C6D1DE` / `#E2EDFB`)**: Used selectively for high-priority interactive states, primary CTAs, active status indicators, and key metrics.
- **Secondary Bronze / Warm Accent (`#E2C0A9` / `#5A4230`)**: Reserved for link hovers, active tab highlights, and focused border outlines.
- **Tertiary Dark Muted (`#7F7D92` / `#EDE9FF`)**: For supportive highlights, alternate tags, and distinct categorical contrast.
- **Text & Content**:
  - **High-Contrast Primary (`#E6E1E7`)**: Delivering strong contrast ratios against the background for effortless scanning of technical copy and headers.
  - **Muted Secondary (`#C4C6CB`)**: Passing WCAG AA standards for secondary metadata, timestamps, tech stack tags, and sub-labels.
...
- **Tablet (768px – 1023px)**: 8-column grid with `1.25rem` gutters and `1.5rem` canvas margins. Side-by-side modules compress to single or balanced double column layouts.
...
### Depth Hierarchy
- **Base Canvas Level 0**: Foundation viewport, page backgrounds, and deep gutters (`#141317`).
- **Surface Level 1**: Structural cards, resume sections, timeline containers, and code snippet backgrounds (`#1c1b1f` / `#201f23`). Enclosed by a crisp 1px solid structural border (`#44474b`).
- **Surface Level 2**: Interactive items, nested code sub-blocks, hover highlights, table headers, and input controls (`#2b292e`).
...
### Buttons
- **Primary**: Background styled in `#C6D1DE`, text `#27313B`, font `Inter` 14px Semi-Bold, border none, border-radius `4px`, padding `8px 16px`. Hover state shifts background tones accordingly. Focus: solid offset outline.
- **Secondary / Outline**: Background styled in surface tones, border `1px solid #44474b`, text `#E6E1E7`, border-radius `4px`, padding `8px 16px`.
...
- Construction: Background structured surfaces (`#201f23`), border `1px solid #44474b`, padding `2px 8px`, border-radius `4px`.

### Cards & Project Showcases
- Surface: Structured background (`#201f23`) with a solid `1px` border (`#44474b`), radius `8px`, padding `24px`.
...
- Container: Background structured base (`#141317`), border `1px solid #44474b`, radius `6px`.
...
- Fields: Background structured base, border `1px solid #44474b`, radius `4px`, padding `10px 12px`.
