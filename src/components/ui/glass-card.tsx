...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  (
    ...
    const defaultSocials = [
      { icon: Instagram, delay: "400ms", label: "Instagram" },
      { icon: Twitter, delay: "600ms", label: "Twitter" },
      { icon: Github, delay: "800ms", label: "Github" },
    ];
    ...
    return (
      <div
        ref={ref}
        className={`group h-[300px] w-[290px] [perspective:1000px] select-none ${className}`}
        {...props}
      >
        <div className="relative h-full rounded-[50px] bg-gradient-to-br from-zinc-900 to-black shadow-2xl transition-all duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[box-shadow:rgba(0,0,0,0.3)_30px_50px_25px_-40px,rgba(0,0,0,0.1)_0px_25px_30px_0px] group-hover:[transform:rotate3d(1,1,0,30deg)]">
          ...
          <div className="absolute [transform:translate3d(0,0,26px)] w-[calc(100%-10px)]">
            <div className="px-7 pt-[88px] pb-0">
              <span className="block text-xl font-black text-white leading-tight">
                {title}
              </span>
              <span className="mt-3 block text-[13px] leading-relaxed text-zinc-300 line-clamp-3">
                {description}
              </span>
            </div>
          </div>
          <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between [transform-style:preserve-3d] [transform:translate3d(0,0,26px)]">
            <div className="flex gap-2.5 [transform-style:preserve-3d]">
              {socialItems.map(({ icon: Icon, delay, href, label, onClick }, index) => {
                const btnContent = (
                  <button
                    key={index}
                    type="button"
            ...
            >
              <button
                type="button"
                onClick={onButtonClick}
                className="border-none bg-none text-xs font-bold text-white whitespace-nowrap cursor-pointer hover:text-cream"
              >
                {buttonText}
              </button>
              <ChevronDown className="h-4 w-4 stroke-white flex-shrink-0 ml-0.5" strokeWidth={3} />
            </div>
          </div>
